...
def handle_text(message):
    ...
    elif message.text.lower() == list.lst[2]: # "хэй, баррель"
         answer = "Хэй, "
         log(message, answer)
         bot.send_message(message.chat.id, answer + message.from_user.first_name+ "!")
    ...
    elif message.text.lower() == list.lst[6]: # "новости"
         answer = "тут будет полезная инфа по разным предстоящим событиям"
         log(message, answer)
         bot.send_message(message.chat.id, answer)

    elif message.text.lower() == list.lst[7]: # "батуты"
         answer = "тут будет инфа про батуты"
         log(message, answer)
         bot.send_message(message.chat.id, answer)

    elif message.text.lower() == list.lst[8]: # "правила"
